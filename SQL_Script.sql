-- ABOUT : 
/* The "amazondata" database houses a single table named "sales" containing 20 columns and 1000 rows.
Among these columns, 10 are categorical and 10 are numerical. */
...
FROM
    sales; 
-- There are 6 distinct product line in sales table of database amazondata 
...
-- Conclusion
/* Food and beverages: The top-performing product line is Food and Beverages, evidenced by its highest revenue generation, 
highest rating, and being the second most sold product line.
Recommendation: To sustain this success, it's crucial to maintain ample stock levels to promptly meet customer demand, 
ensuring their satisfaction and fostering loyalty.

Health and beauty: The product line that requires the most improvement is the Health and Beauty product line. This conclusion is 
based on its least revenue generation, lowest number of products sold, and average rating.
Recommendation: To enhance Health and Beauty sales, we can gather customer feedback to understand preferences and introduce 
innovative products aligned with market trends.
                      
Home and lifestyle: After Health and Beauty, the Home and Lifestyle product line warrants improvement due to its second-lowest 
revenue generation and sales quantity, along with the lowest ratings.
Recommendation: We can gather feedback from customers to continuously refine and improve the product offerings and overall shopping experience.

Sports and travel: Despite being the second-highest revenue-generating product line, its the third least sold product line and 
has the second-lowest rating.
Recommendation: We can analyze customer feedback, improve marketing strategies, and consider introducing new products.

Electronic accessories and Fashion accessories: For both product lines, the performance is average.
Recommendation: Utilize customer insights to refine product offerings and implement targeted marketing strategies to maximize performance */
...
/* Highest revenue by Electronic accessories is generated in the month of January. 
Highest revenue by Fashion accessories is generated in the month of January. 
Highest revenue by Food and beverages is generated in the month of February. 
Highest revenue by Health and beauty is generated in the month of March.
Highest revenue by Home and lifestyle is generated in the month of March.
Highest revenue by Sports and travel is generated in the month of January. 

Recommendation: Consider introducing new products or enhancing existing ones to meet customer demand during these periods. 
Implement targeted marketing strategies and special promotions to stimulate sales during off-peak months*/

SELECT product_line, name_of_day, SUM(total) AS total_revenue
FROM sales
GROUP BY product_line, name_of_day
ORDER BY product_line, total_revenue DESC;
/* Highest revenue by Electronic accessories is generated on Thursday. 
Highest revenue by Fashion accessories is generated on Saturday. 
Highest revenue by Food and beverages is generated on Wednesday. 
Highest revenue by Health and beauty is generated on Tuesday.
Highest revenue by Home and lifestyle is generated on Sunday.
Highest revenue by Sports and travel is generated on Friday. 
Recommendation: Implementing targeted marketing campaigns and promotions on specific days corresponding to each product category's highest 
revenue can potentially boost sales and capitalize on peak purchasing patterns. */

SELECT product_line, time_of_day, SUM(total) AS total_revenue
FROM sales
GROUP BY product_line, time_of_day
ORDER BY product_line, total_revenue DESC;
/* The highest revenue across all product lines is consistently generated during the afternoon, reflecting peak sales activity during this time.
Recommendation: Optimizing product displays and offering special deals during off-peak hours can attract customers and boost sales. */

SELECT product_line, SUM(quantity) AS total_quantity
FROM sales
GROUP BY product_line
ORDER BY total_quantity DESC;
/* The most sold product line is Electronic accessories, while the least sold product line is Health and beauty.
Recommendation: To increase sales, consider offering promotions or discounts on Health and beauty products to boost their sales. 
Additionally, enhancing the marketing and visibility of Health and beauty items could attract more customers and drive sales. */
...
/* Half of the customers are male, and the other half are female, with a slightly higher proportion of females.
Recommendation: tailor marketing efforts to align with the distinct preferences and purchasing patterns of male and female customers. */

SELECT payment_method, COUNT(payment_method) AS payment_method_count
FROM sales
GROUP BY payment_method
ORDER BY payment_method_count DESC;
/* Ewallet is the most frequently used mode of payment, followed closely by cash, with credit card being the least utilized.
Recommendation: Implement exclusive offers for Ewallet transactions to boost sales.*/

SELECT city, SUM(total) as total_revenue
FROM sales
GROUP BY city
ORDER BY total_revenue DESC;
/* Highest revenue was generated from Naypyitaw and lowest by Mandalay. 
Recommendation: To increase sales in cities generating less revenue, we can collaborate with local businesses for cross-promotions, 
offer exclusive discounts or incentives to encourage purchases.*/

SELECT branch, SUM(total) AS total_revenue
FROM sales
GROUP BY branch
ORDER BY total_revenue DESC;
/* Highest revenue was generated from Branch C, while the lowest was generated by Branch B.
Suggestion: To increase sales in branches generating less revenue, we can provide staff training to enhance customer service and product knowledge. */

SELECT customer_type, count(customer_type) AS customer_type_count
FROM sales
GROUP BY customer_type
ORDER BY customer_type_count DESC;
/* Half of the customers fall under the Normal category, while the other half are Members, with a slightly higher proportion being members.
Recommendation: launch new offers/discounts with subscriptions */
